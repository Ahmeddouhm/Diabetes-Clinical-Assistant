...
Self-contained — calls core modules directly (no FastAPI needed).
"""
...
import config
from core.ingest import load_index, main as ingest_main
...
# ──────────────────────────────────────────────
@st.cache_resource(show_spinner="Loading models & index (first time may take 1-2 min)...")
def load_system():
    """Load the retriever and LLM once, cache across reruns."""
    # Auto-ingest if index doesn't exist yet
    chroma_dir = config.CHROMA_DIR
    if not chroma_dir.exists() or not any(chroma_dir.iterdir()):
        st.info("Building vector index from PDFs — this only happens once...")
        ingest_main()
...
def query_system(retriever, llm, question: str, top_k: int = 5):
...
# ──────────────────────────────────────────────
# Page config & styles (same as original ui.py)
# ──────────────────────────────────────────────
st.set_page_config(
...
with st.sidebar:
    ...
    chunks = retriever.get_total_chunks()
    st.markdown(f"""
...
# ──────────────────────────────────────────────
# Answer (calls core modules directly)
# ──────────────────────────────────────────────
if st.session_state.asked and st.session_state.question:
    with st.spinner("Searching guidelines..."):
        try:
            data = query_system(retriever, llm, st.session_state.question, top_k)
